...
const register = async (req, res) => {
    try {
        ...
        console.error('Register error 1233:', err);
        res.status(500).json({
            success: false,
            message: 'Lỗi server, vui lòng thử lại sau 123' + err
        });
    }
};
...
const forgotPassword = async (req, res) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Lỗi server, không thể xử lý yêu cầu đặt lại mật khẩu.' });
    }
};
...
const resetPassword = async (req, res) => {
    try {
        ...
        // Kiểm tra lại token trước khi đổi mật khẩu
        const resetToken = await PasswordResetToken.findOne({
            email: email,
            token: token,
        ...
        if (!resetToken) {
            return res.status(400).json({ success: false, message: 'Mã xác thực không hợp lệ hoặc đã hết hạn. Vui lòng yêu cầu lại.' });
        }
        ...
        if (!user) {
            // Trường hợp hiếm gặp nếu user bị xóa sau khi yêu cầu token
