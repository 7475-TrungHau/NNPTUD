...
const createEpisode = async (req, res) => {
    try {
        ...
        let movieData = req.body.movie; // Can be ID string or object
        const thumbnailFile = req.files?.thumbnailFile?.[0];
        const videoFile = req.files?.videoFile?.[0];

        // --- Refined Movie ID Extraction and Validation ---
        let movieId = null;
        if (typeof movieData === 'object' && movieData !== null && movieData._id) {
            movieId = movieData._id.toString(); // Extract from object and ensure string
        } else if (typeof movieData === 'string' && movieData.toLowerCase() !== '[object object]' && mongoose.Types.ObjectId.isValid(movieData)) {
            // Use if it's a string, not '[object Object]', and is a valid ObjectId format
            movieId = movieData;
        }

        // Validate extracted movieId
        if (!movieId) {
            // Handle cases where movieData was invalid (e.g., '[object Object]', null, undefined, or invalid ObjectId format)
        ...
        if (!title || !episode_number || (!videoFile && !video_url)) {
            return res.status(400).json({ success: false, message: 'Thiếu thông tin bắt buộc (title, episode_number, và videoFile hoặc video_url)' });
        }
        ...
        const newEpisode = new Episode({
            movie: movieId, // Use the extracted ID
            title,
            slug,
            description,
            release_date,
...
const updateEpisode = async (req, res) => {
    try {
        ...
        if (movieData && !newMovieId) {
            console.log('Invalid movie ID format:', movieData);
        ...
        // Only validate if movieData was explicitly provided in request
        ...
        if (newMovieId && newMovieId.toString() !== episodeToUpdate.movie.toString()) {
            ...
            if (!movieExists) {
                // If files were uploaded but new movie not found, delete them
                if (thumbnailFile) deleteFile(`/thumbnails/episodes/${thumbnailFile.filename}`);
                if (videoFile) deleteFile(`/videos/${videoFile.filename}`);
                return res.status(404).json({ success: false, message: 'Phim mới không tồn tại' });
            }
            episodeToUpdate.movie = newMovieId; // Use the extracted ID
        }
        ...
        if (episode_number && episode_number !== episodeToUpdate.episode_number) {
            const targetMovieId = newMovieId || episodeToUpdate.movie; // Use new ID if provided, else old one
            const existingEpisode = await Episode.findOne({
                movie: targetMovieId, // Use the correct movie ID for the check
